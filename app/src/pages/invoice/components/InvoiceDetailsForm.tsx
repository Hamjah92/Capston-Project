...
import { InvoiceDetailsType } from 'src/@types/invoice';

type props = {
  isEdit?: boolean;
};
export function InvoiceDetailsForm({ isEdit }: props) {
  ...
  const subtotal = values.invoiceDetails.reduce(
    (acc: any, detail: InvoiceDetailsType) => acc + detail.productQuantity * detail.productPrice,
    0
  );

  const totalTax = values.invoiceDetails.reduce(
    (acc: any, detail: InvoiceDetailsType) =>
  ...
  const handleSelectProduct = useCallback(
    (index: number, product: ProductType) => {
      ...
      const newPrice = parseFloat(product.salesPrice);
      const quantity = values.invoiceDetails[index]?.productQuantity || 1;
      console.log(product.salesTax);
      console.log(product);

      const taxes = JSON.parse(product.salesTax as string) as TaxType[];
      const rate = taxes[0].taxRate;

      setValue(`invoiceDetails[${index}].productPrice`, newPrice);
      setValue(`invoiceDetails[${index}].taxInPer`, rate);
      setValue(`invoiceDetails[${index}].productTotal`, newPrice * quantity);
    },
    [setValue, values.invoiceDetails]
  );

  const handleChangeQuantity = useCallback(
    (index: number, quantity: number) => {
      const price = values.invoiceDetails[index]?.productPrice || 0;

      setValue(`invoiceDetails[${index}].productQuantity`, quantity);
      setValue(`invoiceDetails[${index}].productTotal`, price * quantity);
    },
    [setValue, values.invoiceDetails]
  );
  ...
  const handleChangePrice = useCallback(
    (index: number, price: number) => {
      ...
      setValue(`invoiceDetails[${index}].productPrice`, price);
      setValue(`invoiceDetails[${index}].productTotal`, price * quantity);
    },
    [setValue, values.invoiceDetails]
  );
        ...
        {fields.map((field, index) => (
          <Stack key={field.id} direction="row" alignItems="flex-end" spacing={2}>
            ...
            <RHFAutoCompleteObject
              name={`invoiceDetails[${index}].productId`}
              label="Product"
              options={products}
              size="small"
            ...
            <RHFTextField
              size="small"
              name={`invoiceDetails[${index}].productQuantity`}
              label="Quantity"
              type="number"
            ...
            <RHFTextField
              size="small"
              name={`invoiceDetails[${index}].productPrice`}
              label="Price"
              type="number"
              InputProps={{
            ...
            <RHFTextField
              size="small"
              name={`invoiceDetails[${index}].taxInPer`}
              disabled
              label="Tax"
              type="number"
              InputProps={{
            ...
            <RHFTextField
              ...
              size="small"
              type="number"
              name={`invoiceDetails[${index}].productTotal`}
              label="Total"
              placeholder="0.00"
              InputProps={{
                startAdornment: (
